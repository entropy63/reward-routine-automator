...
// step. Injected via chrome.scripting.executeScript({func}), which serializes
// the function's source, so each function must be fully self-contained: no
// imports, no closures over module state (see ADR-017).
...
export function clickVisualSearchButton() {
...
export function probeVisualSearchTarget(allowFallbacks) {
...
export function attachRandomImageToVisualSearch(imageBase64, mimeType, allowFallbacks) {
  function frameNote() {
