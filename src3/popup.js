...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // lib/settings.js (keyed by each card's data-section attribute). The third
  // build renamed the Stats card to Today and it leads (ADR-019): the day at
  // a glance before any action. Saved orders from before the rename are
  // normalized on load (unknown ids dropped, missing ones slotted at their
  // default spot).
  ...
  // The Today card's earn bar and plan preview (ADR-019), and the two new
  // toggles: the Redeem card's restock watcher and the Settings view's
  // scheduled run.
  ...
  // "the purr" defaults to catppuccin; geist and primer remain.
