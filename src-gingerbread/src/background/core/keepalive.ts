...
// close grace period runs ~8s, both punctuated by long silent waits. A 20s
// interval of any cheap extension API call resets the idle timer (ADR-001).
//
// Counted holds, because the waits overlap: the tail of a batch can be closing
// its tabs (a hold) while the next startup step is already running (the
// batch's own keep-alive). The count — not a boolean — is what lets
...
// whenever a batch ends, which would otherwise cut short the tab-closing grace
// period running alongside it.
...
export function holdKeepAlive(): void {
  keepAliveHolds++
  startKeepAlive()
}
