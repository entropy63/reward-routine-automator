...
// body. @ts-nocheck because it runs against the live page (not the worker's
// lib.dom view) and is validated by browser/live runs, not the type-checker;
// the typed signature below is still exported for callers. Any change here
// needs the same caution the original needed.
//
...
export function performHumanTypedSearchOnBing(query: string): Promise<number> {
