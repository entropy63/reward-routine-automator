...
export function readRedeemOptions(query, timeoutMs) {
  return new Promise(resolve => {
    ...
    // Named so QA can exercise it against a fixture: false = marked sold
    ...
    function readAll() {
      ...
      Array.from(document.querySelectorAll("a")).forEach(card => {
        ...
        bySku.set(key, {
          ...
          points,
          available,
          // Absolute, so a stored href is navigable as-is — the dashboard's
          // coin cards open it directly (OPEN_REDEEM_PAGE). The card's own
          // href is "/redeem/sku/…" relative; the key above (the query-
          // stripped path) is reused rather than re-derived.
          href: skuHref ? "https://rewards.bing.com" + key : null
        });
      });
    ...
    function poll() {
      ...
      if (options.length || Date.now() >= deadline) {
        ...
        // relays the dump into the popup's Activity view.
        let dump = "";
        if (!options.length) {
...
// The real page (verbatim capture 2026-09-03, .claude/html-references/
// overwatch-redeem-page.html) has
