...
(() => {
  ...
  try {
    ...
    // The accent color (ADR-020) joins the palette pre-paint too: an accent
    // picked yesterday must not flash the theme's own --brand first.
    const accent = localStorage.getItem("meowAccent");
    if (accent) root.style.setProperty("--brand", accent);

    // The app shell (ADR-020): the window is a fixed-height frame, so the
    // active tab has to be on the root before the first paint or every open
    // flashes the Today view before the remembered one arrives.
    const view = localStorage.getItem("meowView");
    root.dataset.view = view || "today";

    // The popup always opens on the tab views; the Settings view is one gear
