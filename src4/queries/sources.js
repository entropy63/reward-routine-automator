...
// one console.warn before the next source takes over. The local generator
// can never fail, so it rounds off the chain as a source like the others
// rather than a special case after the loop.
...
export function trimQuery(text) {
...
async function fetchGoogleTrendsQuery() {
  ...
  if ((cached && cached.day) !== today || !titles.length) {
    ...
    // Both geos at once: these are independent endpoints, so the cache-miss
    // day pays one round trip, not two. Promise.all preserves geo order, and
    // any failure still throws before anything is stored — the cache stays
    // absent and the next query retries.
...
// popup.html mirrors these ids in each row's data-source attribute.
...
export function normalizeQuerySourceOrder(order) {
  return slotMissingDefaults(order, DEFAULT_QUERY_SOURCE_ORDER);
}

export async function generateMeaningfulQuery() {
