...
// first keystroke, the batch paid the whole chain's latency every search,
// only to then idle through a 5-15s inter-search delay. The prefetch starts
// when the current search is done (and at batch start), fills that idle
// window, and the next tick consumes it warm. Module-level state, so an
// evicted worker just forgets it and the next tick fetches cold — the old
// behavior, not worse.

import { generateMeaningfulQuery } from "./sources.js";

const RECENT_QUERY_MEMORY = 200;

// Repeat queries don't earn points, so keep a short history and avoid them.
export async function nextQuery() {
...
export function startQueryPrefetch() {
...
export async function awaitedQuery() {
...
let generate = generateMeaningfulQuery;
export function setQueryGenerator(fn) {
  generate = fn;
}
