...
export async function refreshStats() {
  ...
  try {
    ...
    if (found) {
      ...
      // The points history (ADR-020): one balance snapshot per local day —
      // the day's first and its latest — so the popup can show earned-today
      // and a trend line. Pure math in pure/history.js; this is the only
      // write. A read without a balance (the one card that didn't answer)
      // records nothing, keeping the last good snapshot.
