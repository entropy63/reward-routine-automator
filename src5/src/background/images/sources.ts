...
// thecatapi (cats), and a locally drawn landscape that needs no network at all
// — a dead image host can never block the step.

import { fetchWithTimeout } from '../queries/sources.ts'
...
// Either a usable image (with the source that produced it) or an error string
// naming every source that failed — the caller checks `base64` vs `error`.
export type ImageFetchResult = { base64?: string; mimeType?: string; source?: string; error?: string }

interface ImageSource {
  name: string
  get: () => Promise<ImagePayload>
}

export function bytesToBase64(uint8: Uint8Array): string {
  let binary = ''
  ...
  for (let i = 0; i < uint8.length; i += chunkSize) {
    binary += (String.fromCharCode as (...codes: number[]) => string).apply(
      null,
...
async function downloadImage(url: string, timeoutMs = 8000): Promise<ImagePayload> {
...
async function fetchPicsumImage(): Promise<ImagePayload> {
  return downloadImage(`https://picsum.photos/600/400.jpg?random=${Date.now()}`)
}

async function fetchCatImage(): Promise<ImagePayload> {
...
// Last resort: draw a landscape locally. No network, no host permission, always
// works, so a dead image host can never block the visual search.
async function generateLocalImage(): Promise<ImagePayload> {
  const width = 600
  const height = 400
...
export async function fetchRandomImageData(): Promise<ImageFetchResult> {
  ...
  for (const source of IMAGE_SOURCES) {
    try {
      ...
      const reason = (e && (e as Error).message) || String(e)
